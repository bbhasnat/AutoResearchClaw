...
Follows ARC's RenderedPrompt / PromptManager pattern but is fully independent:
it does not extend ARC's _DEFAULT_STAGES (which only accepts known stage names).
...
# ---------------------------------------------------------------------------
# Template rendering (same logic as researchclaw.prompts._render)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# RenderedPrompt (re-exported from researchclaw.prompts for convenience)
# ---------------------------------------------------------------------------

from researchclaw.prompts import RenderedPrompt  # noqa: E402
...
class AHVSPromptManager:
    ...
    Supports optional YAML overrides. Compatible with ARC's RenderedPrompt
    output type so it can be used alongside ARC's PromptManager.
    """
