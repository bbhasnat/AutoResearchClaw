...
FRAMEWORK_BUG:
  Symptoms: ImportError, wrong path, truncated CodeAgent output, missing worktree
  subdir, eval_command crash unrelated to hypothesis code, ENOENT on framework files
  Action: Fix it (see fixing procedure below), report RERUN_NEEDED

HYPOTHESIS_MISS:
  Symptoms: Code ran correctly, metric did not improve or regressed
...
a. Run full test suite BEFORE your fix:
     cd {ARC_DIR} && \
...
e. Run full test suite AFTER your fix:
     cd {ARC_DIR} && \
     {PYTHON} -m pytest tests/ -v \
     --ignore=tests/e2e_docker_sandbox.py --ignore=tests/e2e_real_llm.py \
     2>&1 | tee /tmp/pytest_after_fix.log

f. Compare: all previously-passing tests must still pass (zero regressions)
   Also check the specific regression tests:
...
- "H<N> = FRAMEWORK_BUG. Fixed <description>. Tests pass. RERUN_NEEDED."
